...
def analyze_cached(user_hash: str, user_path: str, ref_path: str, ref_mtime: float):
    ...
    for k in common:
        ua = user_angles[k][idx_user]
        ra = ref_angles[k][idx_ref]
        angle_mae[k] = float(np.nanmean(np.abs(ua - ra)))
...
with tab_editor:
    ...
    ref_upload = st.file_uploader("Загрузить/обновить эталон (mp4/avi/mov/mkv)", type=["mp4","avi","mov","mkv"])

    st.markdown("#### Важность зон (учёт особенностей упражнения)")
    ...
    st.markdown("#### Пороги советов (в градусах)")
        ...
        if st.button("Сохранить", type="primary"):
            if not element_id or " " in element_id:
                ...
                cfg[element_id] = {
                    ...
                    "important_joints": {
                        "hip": float(w_hip), "knee": float(w_knee), "ankle": float(w_ankle),
                        "torso": float(w_torso), "shoulder": float(w_sh), "elbow": float(w_el),
                    },
...
with tab_analyze:
    ...
    if not ref_path:
        st.error("Для упражнения не задан эталон. Добавьте эталон в редакторе.")
        st.stop()
    if not os.path.exists(ref_path):
        st.error(f"Эталон не найден: {ref_path}")
        st.stop()

    user_file = st.file_uploader("Видео пользователя", type=["mp4","avi","mov","mkv"])
    ...
    st.subheader("AI-рекомендации по улучшению")
    tips = generate_ai_recommendations(
        A["angle_mae"], cfg[el], A["user_angles"], A["ref_angles"], A["idx_user"], A["idx_ref"]
    )
    ...
    if not tips:
        ...
        for t in tips:
            ...
            if jkey and jkey in worst_idx:
                ...
                import cv2
                st.image(cv2.cvtColor(frame, cv2.COLOR_BGR2RGB), caption=f"Стоп-кадр: {JOINT_NAMES_RU.get(jkey, jkey)} (красная — вы, зелёная — эталон)")
